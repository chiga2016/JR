...
//        CLIENT_REPOSITORY_MAP.put(playerId, player);
    ...
    public List<Player> readAllOrdered(PlayerOrder order, PlayerFilter playerFilter, String pageNumber, String limit) {
        int pageNumber1=0; int limit1 = 3;
...
//            CLIENT_REPOSITORY_MAP.put(id, player);
    ...
    public boolean validation(Player player) {
        //System.out.println("Валидация " + player);
        try {
            ...
            if(player.getRace()==null){
                //System.out.println("Race пустой");
                return false;
            }
            if(player.getProfession()==null|| player.getProfession().toString().equals("")){

                System.out.println("Profession пустой");
                return false;
            }
        } catch (Exception ex){
            ex.printStackTrace();
